# ATLAS Agent v8.4 - SANS ORDERBY (bug SharePoint)
$script:Version = "8.4"
    ...
    try {
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "8.4" }
...
Write-Host "  ATLAS AGENT v$($script:Version) - SANS ORDERBY" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
