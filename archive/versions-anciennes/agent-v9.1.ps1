# ATLAS Agent v9.1 - FIX CLÉS DUPLIQUÉES JSON (TESTÉ ET VALIDÉ)
$script:Version = "9.1"
...
# ════════════════════════════════════════════════════
# FONCTION CHECK AUTO-UPDATE (FIX CLÉS DUPLIQUÉES)
# ════════════════════════════════════════════════════
    ...
    try {
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "9.1" }
...
Write-Host "  ATLAS AGENT v$($script:Version) - FIX JSON VALIDE" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Log ">>> FIX CLES DUPLIQUEES JSON (TESTE ET VALIDE) <<<" "SUCCESS"
