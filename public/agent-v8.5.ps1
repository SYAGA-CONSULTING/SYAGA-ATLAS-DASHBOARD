# ATLAS Agent v8.5 - VERSION TEST AUTO-UPDATE
$script:Version = "8.5"
...
# ════════════════════════════════════════════════════
# FONCTION CHECK AUTO-UPDATE (SANS ORDERBY)
# ════════════════════════════════════════════════════
function Check-AutoUpdate {
    Write-Log "CHECK AUTO-UPDATE v$($script:Version) - TEST FONCTIONNEL" "UPDATE"
    ...
    try {
        ...
        # IMPORTANT: $select obligatoire, PAS de $orderby (bug SharePoint)
        Write-Log "Recherche commandes dans SharePoint..." "DEBUG"
        
        $searchUrl = "https://${siteName}.sharepoint.com/_api/web/lists(guid'$commandsListId')/items?`$select=Id,Title,CommandType,Status,TargetVersion,TargetHostname&`$top=100"
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "8.5" }
...
Write-Host "  ATLAS AGENT v$($script:Version) - TEST AUTO-UPDATE" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Log ">>> VERSION TEST POUR VALIDER L'AUTO-UPDATE <<<" "SUCCESS"
