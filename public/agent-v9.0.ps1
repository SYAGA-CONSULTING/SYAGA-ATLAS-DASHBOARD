# ATLAS Agent v9.0 - MÉTHODE WEBCLIENT AU LIEU D'INVOKE-RESTMETHOD
$script:Version = "9.0"
...
# ════════════════════════════════════════════════════
# FONCTION CHECK AUTO-UPDATE (WEBCLIENT)
# ════════════════════════════════════════════════════
function Check-AutoUpdate {
    Write-Log "CHECK AUTO-UPDATE v$($script:Version) - WEBCLIENT METHOD" "UPDATE"
    ...
    try {
        ...
        foreach ($cmd in $allCommands) {
            if ($count -lt 5) {
                Write-Log "  Cmd: $($cmd.Title) | Type=$($cmd.CommandType) | Status=$($cmd.Status)" "DEBUG"
                $count++
            }
        }
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "9.0" }
...
Write-Host "  ATLAS AGENT v$($script:Version) - WEBCLIENT METHOD" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Log ">>> METHODE WEBCLIENT AU LIEU D'INVOKE-RESTMETHOD <<<" "SUCCESS"
