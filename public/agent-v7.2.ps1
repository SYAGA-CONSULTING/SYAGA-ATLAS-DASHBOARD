# ATLAS Agent v7.2 - REMONTÉE COMPLÈTE DES LOGS VERS SHAREPOINT
$version = "7.2"
...
Write-Log "[v7.2] TOUS LES LOGS ENVOYÉS VERS SHAREPOINT POUR ANALYSE" "UPDATE"
...
function Send-LogsToSharePoint {
    ...
    try {
        ...
        # Envoyer dans la liste ATLAS-Servers comme entrée de log
        ...
        foreach ($log in $script:LogsToSend) {
            $logsText += "[$($log.Level)] $($log.Message)`n"
        }
        
        # Limiter à 250 caractères pour VeeamStatus
        if ($logsText.Length -gt 250) {
            $logsText = $logsText.Substring(0, 247) + "..."
        }
        ...
        Write-Host "[LOG] Logs envoyés vers SharePoint pour analyse" -ForegroundColor Green
        
        # Vider le buffer
        $script:LogsToSend = @()
    } catch {
        Write-Host "[LOG] Impossible d'envoyer logs: $_" -ForegroundColor Yellow
    }
}
    ...
    try {
        ...
        try {
            ...
            foreach ($debugItem in $items.d.results) {
                Write-Log "[UPDATE] Item trouvé: ID=$($debugItem.Id), Title=$($debugItem.Title), Status=$($debugItem.Status), Target=$($debugItem.TargetHostname)" "DEBUG"
            }
        } catch {
