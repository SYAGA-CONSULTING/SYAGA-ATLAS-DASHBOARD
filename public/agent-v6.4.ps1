# ATLAS Agent v6.4 - Test UPDATE_ALL + Logs + Rollback
$version = "6.4"
...
Write-Log "[v6.4] Test complet UPDATE_ALL avec logs et rollback" "UPDATE"
    ...
    try {
        ...
        foreach ($item in $items.d.results) {
            if ($item.Title -eq "UPDATE_COMMAND_$hostname" -or $item.Title -eq "UPDATE_ALL") {
                ...
                Write-Log "[UPDATE_ALL] Commande détectée pour mise à jour globale" "UPDATE"
