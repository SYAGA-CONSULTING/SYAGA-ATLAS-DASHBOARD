# ATLAS Agent v8.1 - TEST AUTO-UPDATE + MOINS DE DEBUG
$script:Version = "8.1"
...
function Check-AutoUpdate {
    ...
    try {
        ...
        Write-Log "Trouve $($commands.Count) commande(s) PENDING" "INFO"
        ...
        foreach ($cmd in $commands) {
            if ($cmd.CommandType -eq "UPDATE_ALL" -or $cmd.CommandType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "8.1" }
