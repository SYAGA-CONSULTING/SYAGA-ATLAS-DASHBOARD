...
$i=$e.value|Where{$_.fields.Hostname -eq $H}
if($i){Invoke-RestMethod -Uri "$u/$($i.id)" -Headers $h -Method PATCH -Body (@{fields=$m}|ConvertTo-Json) -ContentType "application/json"}else{$m.Title=$H;Invoke-RestMethod -Uri $u -Headers $h -Method POST -Body (@{fields=$m}|ConvertTo-Json) -ContentType "application/json"}
Write-Host "[$H] v0.19"