# ATLAS Agent v5.8 - Test Auto-Update avec Rollback Task
$version = "5.8"
...
function Write-Log {
    ...
    $color = @{INFO="White"; OK="Green"; ERROR="Red"; UPDATE="Cyan"; WARNING="Yellow"; v53="Magenta"; v57="Cyan"}[$Level]
    Write-Host $log -ForegroundColor $color
}

Write-Log "===== Agent ATLAS v$version DEMARRE =====" "OK"

# Vérifier si on vient d'un update
$rollbackTask = Get-ScheduledTask -TaskName "SYAGA-ATLAS-Rollback" -EA SilentlyContinue
if ($rollbackTask) {
    Write-Log "[ROLLBACK] Tâche rollback détectée - Update récent" "WARNING"
    Write-Log "[ROLLBACK] Test heartbeat pour confirmer stabilité..." "INFO"
    # Si on arrive ici sans crash, c'est bon
    Cancel-RollbackTask
    Write-Log "[UPDATE] v$version STABLE - Rollback annulé" "OK"
}

# FONCTION AUTO-UPDATE v5.3 - Améliorée
# FONCTION ROLLBACK TASK
function Create-RollbackTask {
    param([string]$FromVersion, [string]$ToVersion)
    
    Write-Log "[ROLLBACK] Création tâche rollback (timeout: $rollbackTimeout min)" "WARNING"
    
    $rollbackScript = @"
# ROLLBACK AUTOMATIQUE
`$logFile = 'C:\SYAGA-ATLAS\rollback.log'
'Rollback de v$ToVersion vers v$FromVersion' | Out-File `$logFile -Append
Invoke-RestMethod -Uri 'https://white-river-053fc6703.2.azurestaticapps.net/public/agent-v$FromVersion.ps1' | Out-File 'C:\SYAGA-ATLAS\agent.ps1' -Force
'Rollback terminé' | Out-File `$logFile -Append
Unregister-ScheduledTask -TaskName 'SYAGA-ATLAS-Rollback' -Confirm:`$false
"@
    
    `$rollbackScript | Out-File "$configPath\rollback.ps1" -Encoding UTF8 -Force
    
    try {
        Unregister-ScheduledTask -TaskName "SYAGA-ATLAS-Rollback" -Confirm:$false -EA SilentlyContinue
        
        $action = New-ScheduledTaskAction -Execute "PowerShell.exe" `
            -Argument "-ExecutionPolicy Bypass -File `"$configPath\rollback.ps1`""
        ...
        Write-Log "[ROLLBACK] Tâche créée - Déclenchement dans $rollbackTimeout min" "OK"
        return $true
    } catch {
...
function Cancel-RollbackTask {
    Write-Log "[ROLLBACK] Annulation tâche - Update OK" "OK"
    try {
        Unregister-ScheduledTask -TaskName "SYAGA-ATLAS-Rollback" -Confirm:$false -EA SilentlyContinue
        Remove-Item "$configPath\rollback.ps1" -Force -EA SilentlyContinue
        return $true
    } catch {
        return $false
    }
...
function Check-Update {
    Write-Log "Check auto-update v5.7..." "UPDATE"
            ...
            if ($item.Title -eq "UPDATE_COMMAND_$hostname" -or $item.Title -eq "UPDATE_ALL") {
                ...
                try {
                    ...
                    $updateLog = @{
                        ...
                        Role = "Log"
                        VeeamStatus = "v$version -> v$targetVersion @ $(Get-Date -Format 'HH:mm:ss')"
...
try {
    ...
    if ($existingItem) {
        ...
        Write-Log "[v5.7] UPDATE OK - $statusText" "OK"
    } else {
        # CREATE
        $createUrl = "https://syagacons.sharepoint.com/_api/web/lists(guid'$listId')/items"
        $headers["Content-Type"] = "application/json;odata=verbose"
        ...
        Write-Log "[v5.7] CREATE OK - $statusText" "OK"
    }
