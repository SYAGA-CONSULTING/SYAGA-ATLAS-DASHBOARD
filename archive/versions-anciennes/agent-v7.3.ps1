# ATLAS Agent v7.3 - LOGS SÉPARÉS POUR ÉVITER TRONCATURE 250 CHAR
$version = "7.3"
...
function Write-Log {
    ...
    # v7.2 - ENVOYER TOUS LES LOGS IMPORTANTS VERS SHAREPOINT
    if ($Level -in @("ERROR", "UPDATE", "WARNING", "ROLLBACK", "DEBUG")) {
        $script:LogsToSend += @{
            Time = $ts
            Level = $Level 
            Message = $Message
...
# Buffer pour les logs à envoyer
$script:LogsToSend = @()
...
Write-Log "[v7.3] LOGS ENVOYÉS UN PAR UN POUR ÉVITER TRONCATURE" "UPDATE"
...
# FONCTION ENVOI LOGS VERS SHAREPOINT - v7.3 UN LOG PAR ENTRÉE
function Send-LogsToSharePoint {
    param([string]$Context = "General")
    
    if ($script:LogsToSend.Count -eq 0) { return }
    ...
    try {
        ...
        # v7.3 - ENVOYER CHAQUE LOG IMPORTANT DANS UNE ENTRÉE SÉPARÉE
        $listId = "94dc7ad4-740f-4c1f-b99c-107e01c8f70b"
        $hostname = $env:COMPUTERNAME
        $counter = 0
        
        foreach ($log in $script:LogsToSend) {
            # Envoyer seulement les logs importants (ERROR, UPDATE avec info)
            if ($log.Level -eq "ERROR" -or 
                ($log.Level -eq "UPDATE" -and ($log.Message -match "commande|PENDING|DETECTE|trouvée")) -or
                ($log.Level -eq "DEBUG" -and ($log.Message -match "Item trouvé|URL|Nombre total")))
            {
                $counter++
                $data = @{
                    __metadata = @{ type = "SP.Data.ATLASServersListItem" }
                    Title = "LOG${counter}_${hostname}_${Context}"
                    Hostname = $hostname
                    State = "LOG_$($log.Level)"
                    AgentVersion = "v$version"
                    VeeamStatus = "[$($log.Time)] $($log.Message)".Substring(0, [Math]::Min(250, "[$($log.Time)] $($log.Message)".Length))
                ...
                Invoke-RestMethod -Uri $createUrl -Headers $headers -Method POST -Body ($data | ConvertTo-Json -Depth 10) -EA SilentlyContinue
            }
        }
        
        Write-Host "[LOG] $counter logs importants envoyés vers SharePoint" -ForegroundColor Green
        
        # Vider le buffer
        $script:LogsToSend = @()
    } catch {
        Write-Host "[LOG] Erreur envoi: $_" -ForegroundColor Yellow
    }
}

# FONCTION AUTO-UPDATE - v7.2 AVEC LOGS DÉTAILLÉS
function Check-Update {
    Write-Log "===== CHECK AUTO-UPDATE v$version DÉMARRÉ =====" "UPDATE"
    Write-Log "[UPDATE] Liste ATLAS-Commands ID: ce76b316-0d45-42e3-9eda-58b90b3ca4c5" "DEBUG"
    ...
    try {
        ...
        Write-Log "[UPDATE] URL de recherche: $searchUrl" "DEBUG"
        Write-Log "[UPDATE] Recherche commandes avec Status=PENDING" "UPDATE"
        ...
        try {
            ...
            Write-Log "[UPDATE] Réponse SharePoint reçue" "DEBUG"
            Write-Log "[UPDATE] ☞☞☞ NOMBRE TOTAL D'ITEMS: $($items.d.results.Count) ☜☜☜" "UPDATE"
            
            # Logger les détails de chaque item
            foreach ($debugItem in $items.d.results) {
                Write-Log "[UPDATE] ITEM: ID=$($debugItem.Id), Title=$($debugItem.Title), Status=$($debugItem.Status), Target=$($debugItem.TargetHostname), Type=$($debugItem.CommandType)" "UPDATE"
            }
        } catch {
            Write-Log "[UPDATE] ❌ ERREUR ACCÈS LISTE: $_" "ERROR"
            Write-Log "[UPDATE] URL tentée: $searchUrl" "ERROR"
            Send-LogsToSharePoint -Context "UPDATE_ERROR"
            return
        }
        ...
        Send-LogsToSharePoint -Context "UPDATE_EXCEPTION"
    }
    
    Write-Log "===== FIN CHECK UPDATE =====" "UPDATE"
...
Send-LogsToSharePoint -Context "SESSION_END"
