...
$h=@{Authorization="Bearer $($t.access_token)";'Content-Type'='application/json; charset=utf-8'}
...
# Récupérer tous les items sans filter (évite erreur 400)
$e=Invoke-RestMethod -Uri "$u`?`$expand=fields" -Headers $h
$myItem=$e.value|Where-Object{$_.fields.Hostname -eq $H}|Select-Object -First 1

if($myItem){
    $id=$myItem.id
    $json=@{fields=$m}|ConvertTo-Json -Depth 10
    $jsonBytes=[System.Text.Encoding]::UTF8.GetBytes($json)
    Invoke-RestMethod -Uri "$u/$id" -Headers $h -Method PATCH -Body $jsonBytes -ContentType "application/json; charset=utf-8"|Out-Null
    ...
    try{
        $json=@{fields=$m}|ConvertTo-Json -Depth 10
        $jsonBytes=[System.Text.Encoding]::UTF8.GetBytes($json)
        Invoke-RestMethod -Uri $u -Headers $h -Method POST -Body $jsonBytes -ContentType "application/json; charset=utf-8"|Out-Null
