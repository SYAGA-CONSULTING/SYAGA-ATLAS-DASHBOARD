# ATLAS Agent v8.3 - FIX FINAL REQUÊTE AUTO-UPDATE
$script:Version = "8.3"
...
# ════════════════════════════════════════════════════
# FONCTION CHECK AUTO-UPDATE (FIX ULTIME)
# ════════════════════════════════════════════════════
function Check-AutoUpdate {
    ...
    try {
        ...
        foreach ($cmd in $allCommands) {
            Write-Log "Commande: Type=$($cmd.CommandType) Status=$($cmd.Status) Target=$($cmd.TargetHostname) Version=$($cmd.TargetVersion)" "DEBUG"
        }
        ...
        foreach ($cmd in $allCommands) {
            ...
            if ($cmd.Status -and $cmd.Status -eq "PENDING") {
                Write-Log "Commande PENDING trouvee: $($cmd.CommandType)" "DEBUG"
                
                # Vérifier que c'est pour ce serveur ou ALL
                $targetHost = if ($cmd.TargetHostname) { $cmd.TargetHostname } else { "" }
                
                if ($targetHost -eq "ALL" -or $targetHost -eq $hostname -or $targetHost -eq "") {
                    $pendingCommands += $cmd
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "8.3" }
...
Write-Host "  ATLAS AGENT v$($script:Version) - FIX FINAL AUTO-UPDATE" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
