# ATLAS Agent v5.3 - Direct Installation Ready
$version = "5.3"
...
function Write-Log {
    ...
    $color = @{INFO="White"; OK="Green"; ERROR="Red"; UPDATE="Cyan"; WARNING="Yellow"; v53="Magenta"}[$Level]
    Write-Host $log -ForegroundColor $color
}

Write-Log "===== Agent ATLAS v$version DEMARRE =====" "v53"

# FONCTION AUTO-UPDATE v5.3 - Améliorée
function Check-Update {
    Write-Log "Check auto-update v5.3..." "UPDATE"
    ...
    try {
        ...
        foreach ($item in $items.d.results) {
            if ($item.Title -eq "UPDATE_COMMAND_$hostname" -or $item.Title -eq "UPDATE_ALL") {
                Write-Log ">>> UPDATE DETECTE: $($item.Title) <<<" "v53"
                ...
                try {
                    ...
                    Write-Log "!!! AGENT MIS A JOUR VERS v$targetVersion !!!" "v53"
