...
function Test-RollbackCapability {
    ...
    try {
        ...
        Register-ScheduledTask "SYAGA-ATLAS-Test" -Action $action -Trigger $trigger -Principal $principal | Out-Null
        Unregister-ScheduledTask -TaskName "SYAGA-ATLAS-Test" -Confirm:$false
        
        Write-Log "[ROLLBACK] ✅ Test création tâche: OK" "OK"
    } catch {
        Write-Log "[ROLLBACK] ❌ Test création tâche: ECHEC - $_" "ERROR"
        return $false
    }
    
    # Test 2: Vérifier que le script de rollback peut être exécuté
    try {
        $testScript = "Write-Output 'Test rollback script'"
        $testPath = "$configPath\test_rollback.ps1"
        $testScript | Out-File $testPath -Encoding UTF8 -Force
        
        $result = & PowerShell.exe -ExecutionPolicy Bypass -File $testPath 2>&1
        Remove-Item $testPath -Force
        
        if ($result -eq "Test rollback script") {
            Write-Log "[ROLLBACK] ✅ Test exécution script: OK" "OK"
        } else {
            Write-Log "[ROLLBACK] ❌ Test exécution script: Output inattendu" "ERROR"
            return $false
        }
    } catch {
        Write-Log "[ROLLBACK] ❌ Test exécution script: ECHEC - $_" "ERROR"
        return $false
    }
    
    # Test 3: Vérifier accès au fichier agent.ps1
    if (Test-Path "$configPath\agent.ps1") {
        Write-Log "[ROLLBACK] ✅ Test accès agent.ps1: OK" "OK"
    } else {
        Write-Log "[ROLLBACK] ❌ Test accès agent.ps1: Fichier introuvable" "ERROR"
        return $false
    }
    
    Write-Log "[ROLLBACK] ✅ TOUS LES TESTS REUSSIS - Rollback opérationnel" "OK"
    return $true
}
...
function Create-RollbackTask {
    ...
    Write-Log "[ROLLBACK] *** CRÉATION TÂCHE SYAGA-ATLAS-Rollback ***" "ROLLBACK"
    Write-Log "[ROLLBACK] Timeout: $rollbackTimeout minutes" "ROLLBACK"
...
function Check-Update {
    ...
    try {
        ...
        # Chercher commandes UPDATE - v6.5 avec FILTRAGE
        $hostname = $env:COMPUTERNAME
        $listId = "94dc7ad4-740f-4c1f-b99c-107e01c8f70b"
        
        # IMPORTANT: Filtrer pour ne récupérer QUE les commandes UPDATE
        # Sinon on récupère 100+ entrées et on rate UPDATE_ALL
        $filter = "`$filter=Title eq 'UPDATE_ALL' or Title eq 'UPDATE_COMMAND_$hostname'"
        $searchUrl = "https://syagacons.sharepoint.com/_api/web/lists(guid'$listId')/items?$filter&`$top=10&`$orderby=Modified desc"
        
        Write-Log "[UPDATE] Recherche commandes: UPDATE_ALL ou UPDATE_COMMAND_$hostname" "UPDATE"
        ...
        Write-Log "[UPDATE] $($items.d.results.Count) commande(s) trouvée(s)" "DEBUG"
        ...
        foreach ($item in $items.d.results) {
            if ($item.Title -eq "UPDATE_COMMAND_$hostname" -or $item.Title -eq "UPDATE_ALL") {
                ...
                Write-Log "[UPDATE_ALL] ID: $($item.Id) - Version: $($item.AgentVersion)" "UPDATE"
                Write-Log "[UPDATE_ALL] Status: $($item.VeeamStatus)" "UPDATE"
                ...
                try {
                    ...
                    # v7.0 - Marquer la commande comme EXECUTED dans ATLAS-Commands
                    try {
                        ...
                        Write-Log "Commande marquée EXECUTED" "OK"
                    } catch {
                        Write-Log "Note update status: $_" "DEBUG"
                    }
...
function Send-Heartbeat {
    try {
        ...
        # Préparer données - v6.3 SANS CPUPercent qui cause error 400
        $listId = "94dc7ad4-740f-4c1f-b99c-107e01c8f70b"
        $data = @{
