# ATLAS Agent v5.6 - Fix Erreur 400 SharePoint
$version = "5.6"
...
function Write-Log {
    ...
    $color = @{
        ...
        WARNING="Yellow"; v55="Magenta"; v56="Cyan"; CRITICAL="Red"; DEBUG="Gray"
    }[$Level]
...
function Send-Heartbeat {
    ...
    try {
        ...
        $data = @{
            ...
            Role = "Server"
            # MemoryUsedPercent supprimé - cause erreur 400
        }
    ...
    try {
        ...
        foreach ($item in $items.d.results) {
            if ($item.Title -eq "UPDATE_COMMAND_$hostname" -or $item.Title -eq "UPDATE_ALL") {
                Write-Log ">>> UPDATE DETECTE: $($item.Title) <<<" "v56"
...
# MAIN EXECUTION
Write-Log "===== Agent ATLAS v$version DEMARRE =====" "v56"
...
Write-Log "===== Fin exécution v$version =====" "v56"