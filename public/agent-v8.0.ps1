# ATLAS Agent v8.0 - UTILISE LES VRAIS CHAMPS QUI EXISTENT
$script:Version = "8.0"
...
function Check-AutoUpdate {
    ...
    try {
        ...
        Write-Log "Obtention token..." "DEBUG"
        ...
        foreach ($cmd in $commands) {
            if ($cmd.CommandType -eq "UPDATE_ALL" -or $cmd.CommandType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "8.0" }
                ...
                try {
                    ...
                    if (Test-Path $newAgentPath) {
                        ...
                        Write-Log "MISE A JOUR REUSSIE" "SUCCESS"
...
# ════════════════════════════════════════════════════
# FONCTION HEARTBEAT - UTILISE LES VRAIS CHAMPS
# ════════════════════════════════════════════════════
function Send-HeartbeatWithLogs {
    try {
        Write-Log "Envoi heartbeat..." "DEBUG"
        ...
        Write-Log "Memoire: $memUsage%" "DEBUG"
        
        # DISQUE
        $disk = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='C:'"
        $diskFreeGB = [math]::Round($disk.FreeSpace / 1GB, 1)
        Write-Log "Disque libre: $diskFreeGB GB" "DEBUG"
        ...
        Write-Log "Items existants: $($existing.d.results.Count)" "DEBUG"
        ...
        $data = @{
            ...
            CPUUsage = $cpuUsage           # Pourcentage CPU
            MemoryUsage = $memUsage         # Pourcentage memoire
            DiskSpaceGB = $diskFreeGB       # Espace libre en GB
            Logs = $cleanLogs               # Logs nettoyes
        }
        
        Write-Log "Champs prepares:" "DEBUG"
        Write-Log "  CPUUsage=$cpuUsage%" "DEBUG"
        Write-Log "  MemoryUsage=$memUsage%" "DEBUG" 
        Write-Log "  DiskSpaceGB=$diskFreeGB GB" "DEBUG"
        Write-Log "  Logs=$($cleanLogs.Length) chars" "DEBUG"
        ...
        if ($existing.d.results.Count -gt 0) {
            ...
            try {
                ...
                Write-Log ">>> HEARTBEAT + LOGS ENVOYES OK <<<" "SUCCESS"
                Write-Log "Metriques: CPU=$cpuUsage% MEM=$memUsage% DISK=$diskFreeGB GB" "SUCCESS"
            } catch {
                Write-Log "Erreur update: $_" "ERROR"
                if ($_.Exception.Response) {
                    $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
                    $responseBody = $reader.ReadToEnd()
                    Write-Log "Details: $responseBody" "ERROR"
                }
            }
        } else {
            # Create
            ...
            try {
                ...
                Write-Log ">>> HEARTBEAT + LOGS CREES OK <<<" "SUCCESS"
                Write-Log "Metriques: CPU=$cpuUsage% MEM=$memUsage% DISK=$diskFreeGB GB" "SUCCESS"
            } catch {
        ...
        Write-Log "Erreur generale: $_" "ERROR"
    }
}
...
Write-Host "  ATLAS AGENT v$($script:Version) - ENFIN LES BONS CHAMPS !" -ForegroundColor Cyan
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
