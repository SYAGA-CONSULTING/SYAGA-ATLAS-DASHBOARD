# ATLAS Agent v9.2 - TEST AUTO-UPDATE FINAL
$script:Version = "9.2"
...
# ════════════════════════════════════════════════════
# FONCTION CHECK AUTO-UPDATE (IDENTIQUE v9.1)
# ════════════════════════════════════════════════════
function Check-AutoUpdate {
    ...
    try {
        ...
        # WebClient pour récupérer le JSON
        Write-Log "Recherche commandes avec WebClient..." "DEBUG"
        ...
        $searchUrl = "https://syagacons.sharepoint.com/_api/web/lists(guid'ce76b316-0d45-42e3-9eda-58b90b3ca4c5')/items"
        Write-Log "URL: $searchUrl" "DEBUG"
        
        # Télécharger le JSON
        $jsonResponse = $webClient.DownloadString($searchUrl)
        
        # FIX CRITIQUE : SharePoint renvoie "Id" ET "ID" - supprimer les doublons
        Write-Log "Nettoyage cles dupliquees JSON..." "DEBUG"
        ...
        # Debug - Afficher les 3 premières
        $count = 0
        foreach ($cmd in $allCommands) {
            if ($count -lt 3) {
                Write-Log "  Cmd: $($cmd.Title) | Status=$($cmd.Status) | Type=$($cmd.CommandType)" "DEBUG"
                $count++
            }
        }
        ...
        foreach ($cmd in $allCommands) {
            if ($cmd.Status -and $cmd.Status -eq "PENDING") {
                ...
                if ($targetHost -eq "ALL" -or $targetHost -eq $hostname) {
                    $pendingCommands += $cmd
                    Write-Log ">>> COMMANDE DETECTEE: $($cmd.CommandType) v$($cmd.TargetVersion) <<<" "SUCCESS"
                }
            }
        }
        ...
        foreach ($cmd in $pendingCommands) {
            ...
            if ($cmdType -eq "UPDATE_ALL" -or $cmdType -eq "UPDATE") {
                ...
                $newVersion = if ($cmd.TargetVersion) { $cmd.TargetVersion } else { "9.2" }
                ...
                try {
                    ...
                    if (Test-Path $newAgentPath) {
                        ...
                        # Marquer DONE (on prend la première commande UPDATE)
                        if ($cmd.Id) {
                            ...
                            try {
                                ...
                                Write-Log "Commande $($cmd.Id) marquee DONE" "SUCCESS"
                            } catch {
                        ...
                        # On sort immédiatement après la première mise à jour
                        exit 0
                    }
                } catch {
...
Write-Host "  ATLAS AGENT v$($script:Version) - TEST FINAL AUTO-UPDATE" -ForegroundColor Cyan  
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host ">>> AUTO-UPDATE ENFIN FONCTIONNEL ! <<<" -ForegroundColor Green
Write-Host ""
...
Write-Log ">>> TEST FINAL - AUTO-UPDATE VALIDE ! <<<" "SUCCESS"
